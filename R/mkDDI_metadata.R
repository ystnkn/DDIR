...
mkDDI_metadata <- function(title,data.file.name,ddi.file.name,var.ID,var.name,var.qstnLit,var.catgry.catValu,var.catgry.labl){
    ## 'mk.docDscr'
    docDscr.xml <- tibble(title=title) %>%  DDIR::mk_docDscr()
    ## 'mk.stdyDscr'
    stdyDscr.xml <- tibble(title=title) %>% DDIR::mk_stdyDscr()
    ## 'mk.fileDscr'
    fileDscr.xml <- tibble(data.file.name=data.file.name) %>% DDIR::mk_fileDscr()
    ## 'mk.dataDscr'
    dataDscr.xml <- DDIR::mk_dataDscr(var.ID,
                                var.name,
                                var.qstnLit,
                                var.catgry.catValu,
                                var.catgry.labl
